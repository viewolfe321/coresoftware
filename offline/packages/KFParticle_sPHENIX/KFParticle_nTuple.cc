...
void KFParticle_nTuple::initializeBranches(PHCompositeNode* topNode)
{
  ...
  for (int i = 0; i < m_num_tracks_nTuple; ++i)
  {
    ...
    if(m_get_dEdx) m_tree->Branch(TString(daughter_number) + "_calculated_dEdx", &m_calculated_daughter_dedx[i], TString(daughter_number) + "_calculated_dEdx/F"); // m_get_dEdx defaults to false; run get_dEdx_info() to change this
                                   ...
                                   std::vector<KFParticle> intermediates)
{
  ...
  for (int i = 0; i < m_num_tracks_nTuple; ++i)
  {
    ...
    if(m_get_dEdx) m_calculated_daughter_dedx[i] = kfpTupleTools.get_dEdx(topNode, daughterArray[i]); // m_get_dEdx defaults to false; run get_dEdx_info() to change this
    ...
    if (m_calo_info)
    {
      fillCaloBranch(topNode, m_tree, daughterArray[i], i, tempEMCalmatch, vertex_fillbranch);
